...
#查看容器ip和容器存储位置
sudo docker inspect --format="{{.GraphDriver.Data.LowerDir}} {{.NetworkSettings.Networks.bridge.IPAddress}}" 容器ID或容器名
```
...
docker pull centos/mysql-57-centos7
#创建守护进程并运行容器
docker run -di --name=Mysql5.7 -p 3306:3306 --platform linux/amd64 -e MYSQL_ROOT_PASSWORD=root centos/mysql-57-centos7
```
